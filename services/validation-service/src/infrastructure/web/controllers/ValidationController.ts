...
export class ValidationController {
  ...
  async triggerValidation(req: FastifyRequest<{ Body: ValidateExtractionRequest }>, reply: FastifyReply) {
  ...
  async updateConfig(req: FastifyRequest<{ Params: { tenantId: string }, Body: ValidationConfigUpdateRequest }>, reply: FastifyReply) {
    const tenantIdFromHeader = req.headers['x-tenant-id'] as string;
    if (tenantIdFromHeader && tenantIdFromHeader !== req.params.tenantId) {
      return reply.status(403).send({ error: 'Tenant mismatch' });
    }
    
    const config = await this.configProvider.updateValidationConfig(req.params.tenantId, req.body);
    return reply.status(200).send(config);
  }
}
